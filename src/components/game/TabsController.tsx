      ...
      >
        {tabsList.map(tab => (
          <TabsTrigger 
            key={tab.id}
            value={tab.id} 
            className={`${isMobile ? "text-xs py-3" : "text-sm py-3"} data-[state=active]:bg-primary/20 data-[state=active]:text-primary transition-all duration-300`}
          >
            <tab.icon className={`${isMobile ? "mr-2 h-4 w-4" : "mr-2 h-4 w-4"}`} />
            {tab.label}
          </TabsTrigger>
        ))}
      </TabsList>
